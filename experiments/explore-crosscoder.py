...
tok: PreTrainedTokenizerFast = PreTrainedTokenizerFast.from_pretrained("austindavis/chessGPT2")
llm: GPT2LMHeadModel = (
    GPT2LMHeadModel.from_pretrained("austindavis/chessGPT2").to(DEVICE).train(False).requires_grad_(False)
)
scc: SparseCrosscoder = (
    SparseCrosscoder.load_from_checkpoint("models/last-v2.ckpt")
    .to(DEVICE)
    .train(False)
    .requires_grad_(False)
)
ds: datasets.Dataset = datasets.load_dataset("austindavis/lichess-uci", "201301", split="train")
...
def collate(record):
    ...
    *_, hidden_states = llm.forward(**text_encoding, output_hidden_states=True).values()
    stacked_hidden_states = torch.stack(hidden_states[1:], dim=1)

    # compute board states
    board_stack = uci_to_board(transcript, as_board_stack=True, map_function=board_to_piece_list)
    ...
    return dict(
        ...
        boards=torch.stack(boards, dim=1).to(DEVICE),
...
# %%
def save_checkpoint(epoch, ds, step, losses, last_checkpoint_index, checkpoint_files, probe):
    global_step = len(ds) * epoch + step
    loss_val = "NA"
    if losses:
        loss_val = f"{losses[-1]['avg_loss']:.5f}"
    filename = f"models/probe-step-{global_step}-loss-{loss_val}.pt"
...
class_weights = torch.tensor(
    [1.0 / class_counts[str(c[0]).lower()] for c in sorted(stoi.items(), key=lambda x: x[1])]
)
...
with open(LOG_FILE, "a") as log:
    for epoch in tqdm(range(NUM_EPOCHS), desc="Epoch", leave=True, total=NUM_EPOCHS):
        ...
        optimizer.zero_grad()
        running_loss = 0
        for step, record in enumerate(tqdm(ds, desc="Step", leave=False)):
            ...
            if (step + 1) % LOG_INTERVAL == 0:
                ...
                running_loss = 0
                global_step = len(ds) * epoch + step
                losses.append(dict(epoch=epoch, step=step, global_step=global_step, avg_loss=avg_loss))
                log.writelines(
                    [
            ...
            if (step + 1) % CHECKPOINT_INTERVAL == 0:
                last_checkpoint_index = save_checkpoint(
                    epoch, ds, step, losses, last_checkpoint_index, checkpoint_files, probe
                )
